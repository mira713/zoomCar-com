...
function appendCars(data){
    ...
    data.forEach((elem,index)=>{
        ...
        main.addEventListener("click", ()=>{
            localStorage.setItem("user-car", JSON.stringify(elem));
...
let startTime = user.pickup.split("T");
let endTime = user.drop.split("T");

start.innerText = startTime[0]+" , "+startTime[0]+" ";
end.innerText = " "+endTime[0]+" , "+endTime[0];
locat.innerText = user.location;
